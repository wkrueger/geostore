...
export class Migration20200310164552 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "dataset" ("id" serial primary key, "store_id" int4 null, "operation_id" int4 null, "media_id" varchar(255) not null, "created_at" timestamptz(0) not null, "extent" varchar(255) null);');
    ...
    this.addSql('create table "mapfile_layer" ("id" serial primary key, "mapfile_id" int4 null, "code" varchar(255) null, "label" varchar(255) not null, "dataset_id" int4 not null, "classes" text not null default \'\');');
    this.addSql('alter table "mapfile_layer" add constraint "mapfile_layer_dataset_id_unique" unique ("dataset_id");');

    this.addSql('alter table "dataset" add constraint "dataset_store_id_foreign" foreign key ("store_id") references "store" ("id") on delete cascade;');
    ...
    this.addSql('alter table "mapfile_layer" add constraint "mapfile_layer_dataset_id_foreign" foreign key ("dataset_id") references "dataset" ("id") on update cascade;');
  }
